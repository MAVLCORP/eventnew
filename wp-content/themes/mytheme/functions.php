...
function my_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'my_content_width', 640 );
}
add_action( 'after_setup_theme', 'my_content_width', 0 );
...
function my_scripts() {
  ...
  wp_enqueue_style('fontawesome');
	...
	wp_enqueue_style( 'my-style', get_stylesheet_uri() );//инициализировали стили

	

	//тут с jQuery разобраться
	//вроде подключил
	wp_enqueue_script('jquery', get_template_directory_uri() . 'js/jQuery.js', array(), '20151215', true);

	wp_enqueue_script( 'my-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'my-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
